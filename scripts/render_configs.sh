...
$VAULT_COMMAND -address=$VAULT_ADDR -field=data -format=json "secret/dsde/firecloud/$ENV/common/firecloud-account.json" >"$INTEGRATION_OUTPUT_LOCATION/user-delegated-sa.json"

# we use the perf testrunner account in all environments
PERF_VAULT_PATH="secret/dsde/terra/kernel/perf/common"
$VAULT_COMMAND -address=$VAULT_ADDR -field=key "$PERF_VAULT_PATH/testrunner/testrunner-sa" | base64 -d > "$INTEGRATION_OUTPUT_LOCATION/testrunner-perf.json"
